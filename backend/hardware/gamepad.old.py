...
#TODO: handle JOYHATMOTION
        ...
        if self.handleMode == 0:
            ...
            eventMsg["code"] = int((self.keymap[code])[1:])
            if eventMsg["type"] == 0:
                eventMsg["value"] = round(int(eventMsg["value"]))
                if self.keys[self.keymap[code]] != bool(eventMsg["value"]):
                    self.keys[self.keymap[code]] = bool(eventMsg["value"])
                else:
                    return
            self.btnQueue.put(eventMsg)
            ...
            for combo in self.keycombos:
                ...
                if pressed != combo.pressed:
                    ...
                    keycomboMsg["slot"] = self.slot
                    keycomboMsg["type"] = 2
                    keycomboMsg["code"] = combo.id
                    if pressed == True:
                        keycomboMsg["value"] = 1
                    else:
                        keycomboMsg["value"] = 0
                    self.btnQueue.put(keycomboMsg)
            else:
                pass
...
#TODO: skip buttons
        #if keymapping mode
        elif self.handleMode == 1 and time.time() > self.lastmap+0.5:
            ...
            if event.type == JOYBUTTONDOWN or (event.type == JOYAXISMOTION and
                ...
                self.lastmap = time.time()
                if eventMsg["type"] == 0 and event.type == JOYAXISMOTION:
                ...
                mappingMsg = {}
                mappingMsg["slot"] = self.slot
                mappingMsg["action"] = 2
                if self.current_mapid < len(cGamepad.gencontrollerButtons)-1:
                    self.current_mapid+= 1
                    mappingMsg["key"] = cGamepad.gencontrollerButtons[self.current_mapid]
                else:
                    self.current_mapid = None
